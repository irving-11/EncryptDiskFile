...
bSuccess=FillConsoleOutputAttribute(hConsole,csbi.wAttributes,dwConSize,coor dScreen,&cCharsWritten);
...
if(pinths64,pdih,pinths64->OptionalHeader.DataDirectory[0].VirtualAddres s==0) goto imp;
...
ppdwAddr                                                        = (PULONGLONG*)ImageRvaToVa((PIMAGE_NT_HEADERS)pinths64,pdih,(DWORD)ppdwAddr,N ULL);
ULONGLONG
...
imp:
printf("\n\n 输入表：\n");
printf("\tHint\tThunkRVA\tName\n"); PIMAGE_IMPORT_DESCRIPTOR piid; PIMAGE_THUNK_DATA _pThunk=NULL; DWORD dwThunk=NULL;
USHORT Hint;
...
for(;piid->Name!=NULL;)
{
        ...
        for (; _pThunk->u1.AddressOfData != NULL;)
        {
            ...
            char
                *szFun = (PSTR)ImageRvaToVa((PIMAGE_NT_HEADERS)pinths64, pdih, (ULONG)(((PIMAGE_ IMPORT_BY_NAME)_pThunk->u1.AddressOfData)->Name), 0);
            if (szFun != NULL)
...
printf("输入文件名（支持文件拖拽，直接按回车则默认打开 ntoskrnl.exe，输入
exit 退出）：");
gets(filename);
